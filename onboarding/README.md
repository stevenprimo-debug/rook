...
| 4 | `agents/<each-of-20>/memory/00-operator-profile.md` (`pin: true`) | Each agent reads the customer's profile from its own memory on session start |
...
└── templates/
    ...
    ├── operator-profile.md                     # → memory/00-operator-profile.md (per agent)
...
## Two top-level inboxes the customer needs to know about

Onboarding teaches the customer about two folders that ARE the I/O contract with ROOK:

### `inbox/` — captures land here (Customer → ROOK)

Anything the customer wants ROOK to ingest. Phone screenshots, voice memos, web clippings, PDFs, audio, video. No tagging required — `inbox-routing` classifies and files automatically.

### `_from_rook/` — artifacts land here (ROOK → Customer)

When the customer wants a markdown file they'll read later in Obsidian (a brief, plan, summary, perplexity-bound doc, meeting prep), they say one of these to any agent:

| Trigger phrase | What ROOK does |
|---|---|
| "Send it to me" | Writes the artifact to `_from_rook/YYYY-MM-DD-<slug>.md` |
| "Save this for me" | Same |
| "Send to ROOK" | Same — explicit brand reinforcement |
| "Put this in my reading queue" | Same |
| "I'll read this later" | Same |
| "Drop this for me to read" | Same |

The folder is the customer's reading queue. Every artifact ROOK generates that's meant for the customer to read (not for an agent to consume) ends up here, date-prefixed, ready to scan in Obsidian.

**Distinct from `agents/librarian/digests/`** — librarian's weekly digests land in librarian's own folder (operational output, librarian-owned), not in `_from_rook/`. Keeps the customer's reading queue uncluttered.

The onboarding interview surfaces both folders in Section 7 (added 2026-05-20) so customers know:
- WHERE to drop captures (`inbox/`)
- WHAT to say to get artifacts back (`_from_rook/` via trigger phrases)
- WHERE the librarian's weekly digest lands (`agents/librarian/digests/`)
