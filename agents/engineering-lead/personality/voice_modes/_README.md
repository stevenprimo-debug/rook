...
The bench-of-three principles (Invention / Manufacturability / Drawing-Rigor) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box Engineering Lead voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Invention / Manufacturability / Drawing-Rigor) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Invention / Manufacturability / Drawing-Rigor debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
