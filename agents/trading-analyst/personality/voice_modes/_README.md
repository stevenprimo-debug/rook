...
The bench-of-three principles (Setup-Rigor / Risk-1% / Posture-Current) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box Trading Analyst voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Setup-Rigor / Risk-1% / Posture-Current) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Setup-Rigor / Risk-1% / Posture-Current debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
