...
The bench-of-three principles (Rigor / Synthesis / Actionability) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box Deep Researcher voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Rigor / Synthesis / Actionability) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Rigor / Synthesis / Actionability debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
