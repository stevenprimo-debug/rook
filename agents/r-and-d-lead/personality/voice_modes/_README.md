...
The bench-of-three principles (Novelty / Learning-Velocity / Kill-Criterion) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box R&D Lead voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Novelty / Learning-Velocity / Kill-Criterion) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Novelty / Learning-Velocity / Kill-Criterion debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
