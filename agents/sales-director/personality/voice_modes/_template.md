---
# === FRONTMATTER (fill these in) ===

# Required: the slug for this voice mode. Must match filename (without .md).
voice_mode_name: <your_mode_name>

# Optional: who/what this voice is inspired by. Academic only; never invoked in output.
inspired_by: <attribution string or null>

# Required: one-line description of the voice character.
register: <one-line register description>

# Required: cadence/rhythm of the voice.
cadence: <one-line cadence description>

# Optional: agent slug. Leave as `sales-director`.
agent: sales-director

# Optional: status — "draft" / "active" / "archived"
status: draft
---

# <Your voice mode name> — Voice Mode

<!--
  Blank scaffold for authoring a new voice mode for the Sales Director agent.

  HOW TO USE:
  1. Copy this file to <your_mode_name>.md (lowercase, underscore-separated).
  2. Fill in the frontmatter above.
  3. Fill in each section below. Delete the instructional comments.
  4. Test by setting {voice_mode} = your_mode_name when invoking the agent.

  NOTE: The bench debate (Hunter / Qualifier / Closer) runs UNDERNEATH every voice mode.
  This file controls HOW the agent sounds — not WHETHER its verdicts are sound.
-->

## Voice spine (the five-bullet summary)

1. **<Tone descriptor>.** <One-paragraph: what the tone sounds like, with an example sentence.>
2. **<Sentence shape descriptor>.** <One-paragraph.>
3. **<Opening move descriptor>.** <One-paragraph.>
4. **<Anti-pattern descriptor>.** <One-paragraph.>
5. **<Closing move descriptor>.** <One-paragraph.>

## Signature phrases

- "<Signature phrase 1>"
- "<Signature phrase 2>"
- "<Signature phrase 3>"
- "<Signature phrase 4>"
- "<Signature phrase 5>"
- "<Signature phrase 6>"

## Do-list

- <Do-item 1>: <Reason.>
- <Do-item 2>: <Reason.>
- <Do-item 3>: <Reason.>
- <Do-item 4>: <Reason.>
- <Do-item 5>: <Reason.>
- <Do-item 6>: <Reason.>

## Don't-list

- <Don't-item 1>: <Reason.>
- <Don't-item 2>: <Reason.>
- <Don't-item 3>: <Reason.>
- <Don't-item 4>: <Reason.>
- <Don't-item 5>: <Reason.>
- <Don't-item 6>: <Reason.>

## Sample paragraphs (3 worked examples)

### Example 1 — Weekly pipeline review

> <Write the sample output in your voice here.>

### Example 2 — Forecast lock

> <Write the sample output in your voice here.>

### Example 3 — Win-loss debrief

> <Write the sample output in your voice here.>

## Edge cases / register guards

- **High-stress user state (`{user_state} = deadline` or `frustrated`):** <How the voice shifts.>
- **Exploratory user state:** <How the voice shifts.>
- **Stage-debate mode:** <How the voice handles 3-pole narration.>

## Voice compatibility check

<Compatibility notes.>

## Attribution (academic only — never invoked in agent output)

<Attribution detail.>

---

## Cross-references

- Out-of-box voice: [`_default.md`](_default.md)
- Customer instructions: [`_README.md`](_README.md)
- Bench composition: [`../_bench.md`](../_bench.md)
- Frameworks index: [`../frameworks_index.md`](../frameworks_index.md)
- Master skill: `../../SKILL.md`
- Voice spine: `.claude/voice-spine.md`
