...
The bench-of-three principles (Clarity / Wit / Utility) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box Copywriter voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Clarity / Wit / Utility) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Clarity / Wit / Utility debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
