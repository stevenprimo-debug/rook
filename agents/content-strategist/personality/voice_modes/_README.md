...
The bench-of-three principles (Editorial-Craft / Direct-Response / Audience-Asset) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box Content Strategist voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Editorial-Craft / Direct-Response / Audience-Asset) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Editorial-Craft / Direct-Response / Audience-Asset debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
