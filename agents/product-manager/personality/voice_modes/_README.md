...
The bench-of-three principles (Jobs-to-be-Done / Scope-Restraint / Shippability) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box Product Manager voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (Jobs-to-be-Done / Scope-Restraint / Shippability) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The Jobs-to-be-Done / Scope-Restraint / Shippability debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
