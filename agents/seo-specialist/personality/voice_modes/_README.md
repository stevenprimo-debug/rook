...
The bench-of-three principles (SERP-Rank / Answer-Engine-Visibility / Topical-Authority) defines WHAT this agent reasons about.
...
| File | Purpose |
|---|---|
| `_default.md` | The out-of-box SEO Specialist voice. Active when `{voice_mode}` is unset or set to `_default`. |
...
The bench debate (SERP-Rank / Answer-Engine-Visibility / Topical-Authority) runs underneath every voice mode — what changes is the surface, not the discipline.
...
Voice modes do NOT change the bench. The SERP-Rank / Answer-Engine-Visibility / Topical-Authority debate runs underneath every voice. What changes is the surface. The bench is structural; the voice is cosmetic (which is the customization layer that makes the agent feel like *yours*).
